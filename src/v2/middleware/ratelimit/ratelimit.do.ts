...
export class RateLimiter {
    ...
    env: Bindings
    app: OpenAPIHono = new OpenAPIHono()

    constructor(state: DurableObjectState, env: Bindings) {
        this.state = state
        this.env = env

        this.app.post("/", async (ctx) => {
            await this.setAlarm()
    ...
    async alarm() {
        ...
        for await (const [key, _value] of values) {
            ...
            const currentWindow = Math.floor(
                this.nowUnix() / parseInt(interval)
            )

            const timestampLessThan = currentWindow - 2
    ...
    async calculateRate(config: Config) {
        ...
        const rate =
            (previousCount * (config.interval - distanceFromLastWindow)) /
                config.interval +
            currentCount
    ...
    getHeaders(blocked: boolean, config: Config) {
        ...
        headers["cache-control"] =
            `public, max-age=${expires}, s-maxage=${expires}, must-revalidate`
        return headers
    }
    ...
    expirySeconds(config: Config) {
        ...
        const secondsRemaining =
            currentWindowEnd * config.interval - this.nowUnix()
